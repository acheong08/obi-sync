...
If you installed this project using [Install with Docker-compose](https://github.com/acheong08/obi-sync/wiki/Server-Installation#docker-compose-option-1), and specified the environment variable `DATA_DIR`, then the files should exist in the same folder you specified the variable to use. By default, vault files are placed in the folder `obi-sync`.

If you installed this project using [Install with Docker](https://github.com/acheong08/obi-sync/wiki/Server-Installation#docker-option-2) or did not specify a variable for `DATA_DIR`, then the files are typically stored in `/var/lib/docker/*`
...
In the section above titled [Install with Docker-compose](https://github.com/acheong08/obi-sync/wiki/Server-Installation#docker-compose-option-1), there are two ways to install obi-sync using `docker-compose`.
1. Single `docker-compose.yml` file
   - See [DOCKER-COMPOSE.YML ONLY](https://github.com/acheong08/obi-sync/wiki/Server-Installation#-docker-composeyml-only)
3. `docker-compose.yml` and `.env` file
   - See [DOCKER-COMPOSE.YML + .ENV](https://github.com/acheong08/obi-sync/wiki/Server-Installation#-docker-composeyml--env)
